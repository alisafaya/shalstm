...
parser.add_argument('--tokenizer', type=str, default='tokenizer/spmodels/pile_32k.model',
                    help='location of sentencepiece tokenizer model')
...
fn = 'sp.corpus.{}.pt'.format(hashlib.md5((args.data + args.tokenizer).encode()).hexdigest())
sp_tokenizer = spm.SentencePieceProcessor(model_file=args.tokenizer, add_eos=True)

if os.path.exists(fn):
    print("Loading", fn)
    datasets = torch.load(fn)
else:
    datasets = []
    for s in ("train", "valid", "test"):
        file = os.path.join(args.data, f"{s}.txt.raw")
        print("Tokenizing", file)
        f = open(file)
        lines = [ l for l in f.read().splitlines() if len(l.strip()) > 0 ]
        tokenized_text = sp_tokenizer.encode(lines)
        data = torch.tensor(list(chain(*tokenized_text)))
        datasets.append(data)

    torch.save(datasets, fn)

corpus_train, corpus_valid, corpus_test = datasets
    
eval_batch_size = min(100, args.batch_size)
print('Eval batch size of', eval_batch_size)
test_batch_size = 8

print('Datasize:', corpus_train.shape[0])
print('Datasize:', corpus_valid.shape[0])
print('Datasize:', corpus_test.shape[0])

train_data = batchify(corpus_train, args.batch_size, args)
val_data = batchify(corpus_valid, eval_batch_size, args)
test_data = batchify(corpus_test, test_batch_size, args)

###############################################################################
# Build the model
###############################################################################

ntokens = sp_tokenizer.vocab_size()
    ...
    while i < train_data.size(0) - 1 - 1:
        ...
        if batch % args.log_interval == 0 and batch > 0:
            ...
            print('| epoch {:3d} | {:5d}/{:5d} batches | lr {:05.5f} | clip {:05.3f} | ms/batch {:5.2f} | '
                    'loss {:5.2f} | ppl {:8.2f} | bpt {:8.3f}'.format(
...
try:
    ...
    for epoch in range(1, args.epochs+1):
        ...
        print('| end of epoch {:3d} | time: {:5.3f}s | valid loss {:5.3f} | '
            'valid ppl {:8.3f} | valid bpt {:8.3f}'.format(
...
print('=' * 89)
print('| End of training | test loss {:5.3f} | test ppl {:8.3f} | test bpt {:8.3f}'.format(
    test_loss, math.exp(test_loss), test_loss / math.log(2)))
print('=' * 89)

for (s, l, d) in (("valid", stored_loss, corpus_valid), ("test", test_loss, corpus_test)):
    file = os.path.join(args.data, f"{s}.txt.raw")
    text_char_len = sum([ len(lin) + 1 for lin in open(file).read().splitlines() if len(lin.strip()) > 0 ])
    print(s, "normalized bpc {:8.3f}".format( (l/ math.log(2)) * ( d.shape[0] / text_char_len ) ,))
