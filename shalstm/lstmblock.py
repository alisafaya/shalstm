...
class LSTMBlock(nn.Module):
    ...
    def __init__(self, input_size, fforward_size, rnn="lstm", dropout=0.1, use_attn=False, device=torch.device("cpu")):
        ...
        self.ln_input = LayerNorm(input_size, eps=1e-12)
        self.ln_queries = LayerNorm(input_size, eps=1e-12) if use_attn else None
        self.ln_values = LayerNorm(input_size, eps=1e-12) if use_attn else None
